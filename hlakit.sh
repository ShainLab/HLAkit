...
usage() {
    cat << EOF
Usage: $0 --resultdir <dir> --normalbam <bam> --tumorbam <bam> --allelefile <file> --reference <string> --hlakit <path>
...
Optional arguements:
    ...
    -p, --tumorpurity <int>   Tumor purity in fraction for running LOH
...
Example:
    $0 --resultdir /path/to/resultdirectory --normalbam normalwes.bam --tumorbam tumorwes.bam --allelefile /path/to/allelefile --reference hg19 --hlakit /path/to/hlakit --format ILM1.8 --threads 8
    $0 -o /path/to/resultdirectory -n normal.bam -t tumor.bam -a /path/to/allelefile -r hg19 -P /path/to/hlakit -f ILM1.8 -p 8 -b 0
EOF
    exit 1
}
...
if [ -z "$resultdir" ] || [ -z "$normalbam" ] || [ -z "$tumorbam" ] || [ -z "$allelefile" ] || [ -z "$reference" ] || [ -z "$hlakit" ]; then
...
sort $allelefile | uniq > $resultdir/allelefile.tmp.txt && mv $resultdir/allelefile.tmp.txt $allelefile
...
run_command "$hlakit"/paired_fastq.sh -f $resultdir/normal_kmer.1.fastq
run_command "$hlakit"/paired_fastq.sh -f $resultdir/normal_kmer.2.fastq
if [[ $? -ne 0 || $(stat -c%s $resultdir/normal_kmer.1.fastq) -le 1 || $(stat -c%s $resultdir/normal_kmer.2.fastq) -le 1 ]]; then
    log_warn "Normal $resultdir/normal_kmer.1.fastq or $resultdir/normal_kmer.2.fastq file is empty."
fi
...
if samtools view -H $normalbam | egrep -q "chr6" ; then
    chr=1
else
    chr=0
fi
...
if [ $reference == "hg38" ]; then
    if [ "$chr" == 1 ]; then
        run_command samtools view --threads $threads $normalbam chr6:29941260-29945884 chr6:31353872-31357187 chr6:31268749-31272105 >> $resultdir/chr6region.normal.sam
    else
        run_command samtools view --threads $threads $normalbam 6:29941260-29945884 6:31353872-31357187 6:31268749-31272105 >> $resultdir/chr6region.normal.sam
    fi
elif [ $reference == "hg19" ]; then
    if [ "$chr" == 1 ]; then
        run_command samtools view --threads $threads $normalbam chr6:29909037-29913661 chr6:31321649-31324964 chr6:31236526-31239869 >> $resultdir/chr6region.normal.sam
    else
        run_command samtools view --threads $threads $normalbam 6:29909037-29913661 6:31321649-31324964 6:31236526-31239869 >> $resultdir/chr6region.normal.sam
    fi
fi
...
run_command "$hlakit"/paired_fastq.sh -f $resultdir/chr6region.normal.1.fastq
run_command "$hlakit"/paired_fastq.sh -f $resultdir/chr6region.normal.2.fastq

log_info "Merging reads from Normal k-mer and chr6 HLA region ..."
run_command "$hlakit"/merge_fastq.sh -k $resultdir/normal_kmer -c $resultdir/chr6region.normal -o $resultdir/normal_subsetted

log_info "SUBSETTING NORMAL BAM DONE!"
...
run_command "$hlakit"/paired_fastq.sh -f $resultdir/tumor_kmer.1.fastq
run_command "$hlakit"/paired_fastq.sh -f $resultdir/tumor_kmer.2.fastq
if [[ $? -ne 0 || $(stat -c%s $resultdir/tumor_kmer.1.fastq) -le 1 || $(stat -c%s $resultdir/tumor_kmer.2.fastq) -le 1 ]]; then
    log_warn "Tumor $resultdir/tumor_kmer.1.fastq or $resultdir/tumor_kmer.2.fastq file is empty."
fi
...
if samtools view -H $tumorbam | egrep -q "chr6" ; then
    chr=1
else
    chr=0
fi
...
then
    if [ "$chr" == 1 ]; then
        run_command samtools view --threads $threads $tumorbam chr6:29941260-29945884 chr6:31353872-31357187 chr6:31268749-31272105 >> $resultdir/chr6region.tumor.sam
    else
        run_command samtools view --threads $threads $tumorbam 6:29941260-29945884 6:31353872-31357187 6:31268749-31272105 >> $resultdir/chr6region.tumor.sam
    fi
elif [ $reference == "hg19" ]; then
    if [ "$chr" == 1 ]; then
        run_command samtools view --threads $threads $tumorbam chr6:29909037-29913661 chr6:31321649-31324964 chr6:31236526-31239869 >> $resultdir/chr6region.tumor.sam
    else
        run_command samtools view --threads $threads $tumorbam 6:29909037-29913661 6:31321649-31324964 6:31236526-31239869 >> $resultdir/chr6region.tumor.sam
    fi
fi
...
run_command "$hlakit"/paired_fastq.sh -f $resultdir/chr6region.tumor.1.fastq
run_command "$hlakit"/paired_fastq.sh -f $resultdir/chr6region.tumor.2.fastq

log_info "Merging reads from Tumor k-mer and chr6 HLA region ..."
run_command "$hlakit"/merge_fastq.sh -k $resultdir/tumor_kmer -c $resultdir/chr6region.tumor -o $resultdir/tumor_subsetted

log_info "SUBSETTING TUMOR BAM DONE!"
log_info "SUBSETTING DONE!"
...
if [ ! -z $format ]; then 
    run_command sh "$hlakit"/runnovoalign.sh --f1 $resultdir/normal_subsetted.1.fastq --f2 $resultdir/normal_subsetted.2.fastq --threads $threads --nixFile $hlakit/resources/hla.nix --output $resultdir/novoalign_normal_reads.sam --hlakit $hlakit --format $format
else 
    run_command sh "$hlakit"/runnovoalign.sh --f1 $resultdir/normal_subsetted.1.fastq --f2 $resultdir/normal_subsetted.2.fastq --threads $threads --nixFile $hlakit/resources/hla.nix --output $resultdir/novoalign_normal_reads.sam --hlakit $hlakit
fi
...
run_command gatk MarkDuplicates -I $resultdir/novoalign_normal.coordsort.RG.bam -O $resultdir/novoalign_normal.coordsort.dedup.RG.bam -M $resultdir/novoalign_normal.coordsort.dedup.RG.metrics.txt --REMOVE_DUPLICATES true
...
if [ ! -z $format ]; then
    run_command sh "$hlakit"/runnovoalign.sh --f1 $resultdir/tumor_subsetted.1.fastq --f2 $resultdir/tumor_subsetted.2.fastq --threads $threads --nixFile $hlakit/resources/hla.nix --output $resultdir/novoalign_tumor_reads.sam --hlakit $hlakit --format $format
else 
    run_command sh "$hlakit"/runnovoalign.sh --f1 $resultdir/tumor_subsetted.1.fastq --f2 $resultdir/tumor_subsetted.2.fastq --threads $threads --nixFile $hlakit/resources/hla.nix --output $resultdir/novoalign_tumor_reads.sam --hlakit $hlakit
fi
...
run_command gatk MarkDuplicates -I $resultdir/novoalign_tumor.coordsort.RG.bam -O $resultdir/novoalign_tumor.coordsort.dedup.RG.bam -M $resultdir/novoalign_tumor.coordsort.dedup.RG.metrics.txt --REMOVE_DUPLICATES true
...
do
    ...
    run_command $hlakit/filterReads.sh -i $resultdir/"${BAM/.bam/.typedsubset.txt}" -o $resultdir/"${BAM/.bam/.typedsubset.cleanSNP.txt}" -m 1

    log_info "Cleaning alignments for DNP detection bam files ..."
    run_command $hlakit/filterReads.sh -i $resultdir/"${BAM/.bam/.typedsubset.txt}" -o $resultdir/"${BAM/.bam/.typedsubset.cleanDNP.txt}" -m 2
...
do
    ...
    do
        ...
        run_command head -2 "$hlasamheader" > $resultdir/${normalBam/.bam/.$allele.sam}
        run_command cat "$hlasamheader" | grep -P "$allele\t" >> $resultdir/${normalBam/.bam/.$allele.sam}
        run_command cp $resultdir/${normalBam/.bam/.$allele.sam} $resultdir/${tumorBam/.bam/.$allele.sam}

        printf "@RG\tID:"$normalsamplename".normal\tPL:illumina\tPU:unit1\tLB:library1\tSM:$resultdir/${normalBam/.bam/.$allele.sorted.bam}\n" >> $resultdir/${normalBam/.bam/.$allele.sam}
        run_command cat $resultdir/$normal | grep -P "\t$allele\t" | sort -k1 > $resultdir/${normalBam/.bam/.$allele.txt}

        if [ ! -s  $resultdir/${normalBam/.bam/.$allele.txt} ];then
            log_warn $resultdir/${normalBam/.bam/.$allele.txt} is empty!!
        fi

        run_command $hlakit/paired_reads_bam.sh -i $resultdir/${normalBam/.bam/.$allele.txt}
        run_command cat $resultdir/${normalBam/.bam/.$allele.txt} >> $resultdir/${normalBam/.bam/.$allele.sam}
        run_command samtools view -bS -o $resultdir/${normalBam/.bam/.$allele.bam} $resultdir/${normalBam/.bam/.$allele.sam}
        run_command samtools sort -o $resultdir/${normalBam/.bam/.$allele.sorted.bam} $resultdir/${normalBam/.bam/.$allele.bam}
        run_command samtools index $resultdir/${normalBam/.bam/.$allele.sorted.bam}

        printf "@RG\tID:"$tumorsamplename".tumor\tPL:illumina\tPU:unit1\tLB:library1\tSM:$resultdir/${tumorBam/.bam/.$allele.sorted.bam}\n" >> $resultdir/${tumorBam/.bam/.$allele.sam}
        run_command cat $resultdir/$tumor | grep -P "\t$allele\t" | sort -k1 > $resultdir/${tumorBam/.bam/.$allele.txt}

        if [ ! -s  $resultdir/${tumorBam/.bam/.$allele.txt} ];then
            log_warn "$resultdir/${tumorBam/.bam/.$allele.txt} is empty!!"
        fi

        run_command $hlakit/paired_reads_bam.sh -i $resultdir/${tumorBam/.bam/.$allele.txt}
        run_command cat $resultdir/${tumorBam/.bam/.$allele.txt} >> $resultdir/${tumorBam/.bam/.$allele.sam}
        run_command samtools view -bS -o $resultdir/${tumorBam/.bam/.$allele.bam} $resultdir/${tumorBam/.bam/.$allele.sam}
        run_command samtools sort -o $resultdir/${tumorBam/.bam/.$allele.sorted.bam} $resultdir/${tumorBam/.bam/.$allele.bam}
        run_command samtools index $resultdir/${tumorBam/.bam/.$allele.sorted.bam}


        log_info "Running Mutect2 on $allele to find $muttype ..."
        if [ ! -s  $resultdir/${tumorBam/.bam/.$allele.txt} ] || [ ! -s $resultdir/${normalBam/.bam/.$allele.txt} ]; then
            log_warn "Both $resultdir/${tumorBam/.bam/.$allele.txt} and $resultdir/${normalBam/.bam/.$allele.txt} are empty. Mutect2 will fail!"
        fi
        run_command gatk Mutect2 \
        -I $resultdir/${normalBam/.bam/.$allele.sorted.bam} \
        -I $resultdir/${tumorBam/.bam/.$allele.sorted.bam} \
        -normal $resultdir/${normalBam/.bam/.$allele.sorted.bam} \
        -O $resultdir/$allele.$muttype.vcf \
        -R $hlakit/resources/hla.fasta
    done
done

log_info "Mutation Calling Done!"

log_info "Running MAPQ correction ..."
run_command sh $hlakit/mapq_correction.sh -o $resultdir -a $resultdir/allelelist_updated.txt -t $threads
log_info "MAPQ correction done!"

log_info "Calculating coverage of each allele in normal and tumor bam files ..."
run_command sh $hlakit/calculate_coverage.sh -o $resultdir -a $resultdir/allelelist_updated.txt -p $hlakit
log_info "Calculating coverage done!"
...
if [ $(wc -l < $resultdir/somatic_mutations.txt) -eq 1 ]; then
    if [ $loh == "yes" ]; then
        log_warn "No mutations found in $tumorbam"
    else
        log_error "No mutations found in $tumorbam". Exiting now.
    fi
else

    log_info "Running samtools mpileup on normal and tumor bam files to count ref and alt alleles ..."
    run_command $hlakit/run_mpileup.sh -o $resultdir -m $resultdir/somatic_mutations.txt -p $hlakit
    log_info "mpileup done!"
    ...
    log_info "Added UV mutations to $resultdir/somatic_mutations.mpileup.UV.txt"

    log_info "Adding features to mutation list ..."
    run_command $hlakit/add_features.sh -i $resultdir/somatic_mutations.mpileup_counts.UV.txt -b $hlakit/resources/hla_bedfile.bed
    log_info "Added features to $resultdir/somatic_mutations.mpileup.UV.features.txt"

    log_info "Annotating mutations ..."
    run_command Rscript $hlakit/annotate_mutations.R -m $resultdir/somatic_mutations.mpileup_counts.UV.features.txt -g $hlakit/resources/hla.gtf -b $hlakit/resources/hla.bed -f $hlakit/resources/hla.fasta
    log_info "Annotation done!"

    log_info "Creating final mutation list ..."
    awk '{print $1"\t"$2"\t"$4"\t"$5"\t"$12"\t"$13"\t"$14"\t"$15"\t"$16"\t"$17"\t"$18"\t"$19"\t"$20"\t"$21"\t"$22"\t"$23"\t"$24}' FS="\t" $resultdir/somatic_mutations.mpileup_counts.UV.features.annotated.txt > $resultdir/hla_somatic_mutations.txt

    # log_info "Short read correction ..."
    # awk 'NR>1{print $1"\t"$2"\t"$3"\t"$4"\t"$17}'  FS="\t" $resultdir/hla_somatic_mutations.txt > $resultdir/read_length_correction.bed
    # run_command sh read_length_correction.sh $resultdir/read_length_correction.bed $resultdir
    # log_info "Short read correction done"
fi
...
if [ "$loh" == "yes" ]; then
    ...
    if [ -z "$tumorpurity" ]; then
        ...
        if [ -z "$WESnormalcoverage" ] || [ -z "$WEStumorcoverage" ]; then
            ...
            run_command Rscript $hlakit/loh.R -p $tumorpurity -n $WESnormalcoverage -t $WEStumorcoverage -c $resultdir/coverage.txt -r $resultdir
        fi
    fi
fi
