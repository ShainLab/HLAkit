# HLAkit
Software to detect somatic point mutations and loss of heterozygosity affecting HLA class I genes.
...
Convert HLA allele names from Standard IMGT format (`A*01:01:01:01`) to flattened format (`hla_a_01_01_01_01`) using R. Also remove the expression status suffix from allele name, for example `A*02:01:01:01N` -> `hla_a_02_01_01_01`.
```
...
## Using subsetted bam files as input:
Set `subsettedbam: yes` to skip BAM subsetting if using pre-subsetted BAMs. HLAkit saves HLA region subsetted bam files for hg19/hg38 aligned reads as normal_subsetted.hg19.bam/tumor_subsetted.hg19.bam in resultdir/bamfiles directory.
